...
if((count (units group player) + count bon_recruit_queue) >= bon_max_units_allowed) exitWith { player commandChat "You've reached the max. allowed group size."; };
...
if(_queuecount > 0) then {
	...
	player commandChat format["%1 added to queue.",_typename];
};
...
player commandChat format["Processing your %1.",_typename];
...
player commandChat format["Your %1 %2 has arrived.", _typename, name _unit];
bon_recruit_queue = bon_recruit_queue - [_queuepos];
