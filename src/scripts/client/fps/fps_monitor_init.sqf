...
fps_marker = createMarkerLocal ["fps_marker",[ 250, 250 ]];
fps_marker setMarkerShapeLocal "ICON";
fps_marker setMarkerTypeLocal "hd_dot";
fps_marker setMarkerColorLocal "ColorYellow";
fps_marker setMarkerTextLocal "";
...
FPSMON_fnc_monitor = {
	...
	_this spawn {
		...
		if (isNil "FPSMON_init" || isNil "FPSMON_clientID") then {
			FPSMON_init = true;
			FPSMON_clientID = nil;
		...
		if ((_delay > 0) && (isNil "FPSMON_handle")) then {
			FPSMON_handle = [_syncTime, _delay] spawn {
				waitUntil {
					...
					_m = format (["Local FPS: %1 - %2 | Server FPS: %3 - %4 / %5 | Headless FPS: %6 - %7 / %8 | Client FPS: %9 - %10 / %11",
						round(diag_fpsmin),
						round(diag_fps)
					] + _output);
					
					fps_marker setMarkerTextLocal _m;
			...
			player commandChat format["FPS Monitoring Started. | %1 Second Interval | Check Map for info.", (_delay + _syncTime)];
			
		} else {
			
			if(!isNil "FPSMON_handle") then {
				terminate FPSMON_handle;
				FPSMON_handle = nil;
			};
			player commandChat "FPS Monitoring Stopped.";
